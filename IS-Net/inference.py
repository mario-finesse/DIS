...
print(f"You are using the following device: {device}")
...
def main(train_datasets,
    ...
    if(hypar["restore_model"]!=""):
        restore_model_path = hypar["model_path"]+"/"+hypar["restore_model"]
        if os.path.exists(restore_model_path):
            print(f"restore model from: {restore_model_path}")
        else:
            raise FileNotFoundError(f"No model at {restore_model_path}")
        if torch.cuda.is_available():
...
if __name__ == "__main__":
    ...
    if hypar["mode"] == "train":
        ...
        hypar["restore_model"] = "isnet-general-use.pth" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing. Include the model name not the file path.
